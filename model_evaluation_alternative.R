###### Model evaluation for when matrix built process is weird when doing one hot encoding for xgBoost

evaluate_models_alternative <- function(data, dataset_name) {
  ...
  data <- data
  X <- data[, -target_col] # nrow 72983
  y <- data[, target_col]  # length(y) 72983

  X <- model.matrix(~ . + 0, data = X)
  nrow(X) # 67212

  trainIndex <- createDataPartition(y[1:  nrow(X)], p = 0.8, list = FALSE)
